...
while(1)
{
	...
	if( flag && n > 0 )
	{
		...
		if(strcmp(buffer,"x\n")==0)
		{
			printf("\nSystem: The other is waiting for your writing!(x to give control, xx to quit)\n");
			flag = false;
		}
		else
		{
	...
	if(!flag && fgets( buffer, BUFSIZE, stdin ) != NULL)
	{
		len = strlen(buffer);
		n = send(client_fd, buffer, len, 0);
		if( n < 0 )
		{
			perror( "client send" );
			exit(1);
